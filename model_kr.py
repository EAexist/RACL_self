...
class MODEL(object):
    ...
    def __init__(self, opt, word_embedding, word_dict):
    ...
    def run(self):
        ...
        unlabel_size = int(len(train_labeled_sets[0])*self.opt.unlabel_ratio)
        print('## unlabel size ##', unlabel_size)
        train_unlabeled_sets = read_data_unlabel(self.opt.unlabel_path, self.word_id_mapping, self.opt.max_sentence_len, unlabel_size)
        ...
        with tf.Session() as sess:
            ...
            # with open('{}'.format('distribution.txt'), 'a', encoding='utf-8') as writer:
            #     writer.write('{}\n{}\n{}\n'.format(np.amax(softmax(pr_a_preds, -1), axis = -1), np.amax(softmax(pr_o_preds, -1), axis = -1), np.amax(softmax(pr_s_preds, -1), axis = -1)))
            #     writer.write('{}\n{}\n{}\n'.format(pred_ay==true_ay, pred_oy==true_oy, pred_sy==true_sy))
        ...
        for j in range(self.opt.num_students):
            with tf.Session() as sess:
                ...
                for i in [np.array(pr_a_preds), np.array(pr_o_preds), np.array(pr_s_preds)]:
                    train_unlabeled_sets_new.append(i)

                if not (self.opt.confidence_thresh == -1):
                    pr_a_mask, pr_o_mask, pr_s_mask = map(prob_to_mask, [pr_a_preds, pr_o_preds, pr_s_preds])

                else:
                    pr_a_mask, pr_o_mask, pr_s_mask = train_unlabeled_sets[7], train_unlabeled_sets[8], train_unlabeled_sets[9]

                for i in [pr_a_mask, pr_o_mask, pr_s_mask]:
                    train_unlabeled_sets_new.append(i)   

                train_unlabeled_sets_new.append(train_unlabeled_sets[-1])
